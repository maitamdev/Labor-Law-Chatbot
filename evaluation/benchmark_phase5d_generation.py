# -*- coding: utf-8 -*-
"""
VietLabor AI - Phase 5D Generation & Evidence Grounding Benchmark
...
- Citation Support Accuracy (Gate: >= 90%)
- Citation Completeness (Gate: >= 90%)
...
def run_phase5d_benchmark(dataset_path: str = "data/evaluation/phase5c_eval_dataset.json"):
    ...
    print("\n" + "=" * 60)
    print("RUNNING 69 IN-SCOPE QUERIES BENCHMARK")
    print("=" * 60)
    ...
    for i, item in enumerate(in_scope_items, 1):
        ...
        # Raw evidence validity tracking from Qwen output
        ...
        # Final visible citation validity
        for cid in cited_cids:
            total_final_visible_citations += 1
            if cid in res.formatted_context.available_chunk_ids:
        ...
        if q_complete:
            ...
            gold_in_retrieved = any(
                any(match_chunk_to_evidence(c, req) for req in missing_reqs)
            ...
            if not gold_in_retrieved:
                ftype = "RETRIEVAL_MISS"
            elif not gold_in_context:
                ftype = "CONTEXT_OMISSION"
            elif len(cited_cids) > 0:
                ftype = "WRONG_CITATION_SELECTION"
            else:
                ftype = "GENERATION_REASONING"
        ...
        # Critical wrong law check: e.g. general employee citing ND145 d7
        ...
        print(f"[{i:02d}/{len(in_scope_items)}] {status_sym} {qid}: {q_text[:45]}... (Evidence: {num_supp}/{num_req}) | RawValid: {raw_validity:.2f}")
    ...
    for c_item in compound_items:
        ...
        print(f"    Cited: {cited}")
        if eval_res["missing_items"]:
            print(f"    Missing: {eval_res['missing_items']}")
    ...
    docs_a = [res_a.formatted_context.chunk_metadata_registry.get(cid, {}).get("doc_id") for cid in cited_a]
    ...
    print(f"  Answer mentions 30d: {'30' in res_a.answer}, Cited arts: {arts_a}")
    ...
    print(f"  Answer mentions 120d: {'120' in res_b.answer}, Cited arts: {arts_b} (Requires BOTH 7 and 35)")
    ...
    print(f"  needs_clarification: {res_c.validated_response.needs_clarification}")
    ...
    out_file = Path("evaluation/results/phase5d_generation_results.json")
    out_file.parent.mkdir(parents=True, exist_ok=True)
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print("\n" + "=" * 60)
    print("PHASE 5D BENCHMARK OVERALL REPORT")
    print("=" * 60)
    ...
    print(f"Citation Support Accuracy      : {fully_supported_questions}/{n_in} = {support_acc_pct:.2f}% (Gate >= 90%)")
    print(f"Citation Completeness          : {total_supported_evidence_items}/{total_required_evidence_items} = {completeness_pct:.2f}% (Gate >= 90%)")
    ...
    print("------------------------------------------------------------")
    print(f"Latencies (ms): Retrieval={summary['latencies']['retrieval']['mean_ms']}, LLM={summary['latencies']['llm']['mean_ms']}, Total={summary['latencies']['total']['mean_ms']}")
